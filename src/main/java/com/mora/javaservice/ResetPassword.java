...
//		String udpateRes = "";
		try {
			String res = DBPServiceExecutorBuilder.builder().withServiceId("DBXDBServices").withOperationId("dbxdb_customer_get")
					.withRequestParameters(requestParam).build().getResponse();
			...
			if (validatePassword(dbPwd, currentPwd)) {
				...
				if(jsonRes.getString("updatedRecords").equals("1")){
					...
					result.addParam("ResponseCode", ErrorCodeMora.ERR_100112.toString());
					result.addParam("Message", ErrorCodeMora.ERR_100112.getErrorMessage());
					logger.debug("Password Reset unsuccessful");
				}
			} else {	
				result.addParam("status", "Current password not correct");
				result.addParam("ResponseCode", ErrorCodeMora.ERR_100111.toString());
				result.addParam("Message", ErrorCodeMora.ERR_100111.getErrorMessage());

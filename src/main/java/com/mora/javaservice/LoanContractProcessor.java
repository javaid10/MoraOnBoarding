...
public class LoanContractProcessor implements JavaService2 {
            ...
            if (preprocess(request, result)) {
                ...
                if (appId != "") {
                    ...
                    if (jsonSchedule.getJSONArray("body").length() > 0) {
                        ...
                        JSONObject nafaesData = new JSONObject(getNafaesData(appId, request, response));
                        ...
                        // 210/01/2023 Nafaes Commodity name changes
                        // inputContract.put("$product_typ_one", "نحاس"); // needs to remove
                        // inputContract.put("$spacification", "نحاس"); // needs to remove

                        inputContract.put("$product_typ_one", "حديد تسليح - سابك");
                        inputContract.put("$spacification", "حديد تسليح - سابك");
                        ...
                        String nafQuantity = "";
                        nafQuantity = nafaesData.optJSONArray("response").optJSONObject(0)
                                .optString("quantity");

                        inputContract.put("$number_weight", nafQuantity);
     ...
     * @return
     */
    private String getNafaesData(String applicationId, DataControllerRequest request, DataControllerResponse response) {
        String resp = "";
        String accessToken = "";
        String refNo = "";
        UUID uuid = UUID.randomUUID();
        ...
        logger.error("getNafaesData applicationid===>> :: " + applicationId);
        ...
        try {
            ...
            if (nafData.getJSONArray("nafaes").length() > 0) {
                ...
                inp.put("orderType", "PO");
                inp.put("lng", "2");
                resp = DBPServiceExecutorBuilder.builder().withServiceId("NafaesRestAPI")
                        .withOperationId("OrderResult_PollingMethod")
                        .withRequestParameters(inp).build()
                        .getResponse();
                String requestJson = new ObjectMapper().writeValueAsString(inp);
                auditLogData(request, response, requestJson, resp);
    ...
    public boolean auditLogData(DataControllerRequest request, DataControllerResponse response, String req, String res)
        ...
        String apiHost = "NAFAES-PO-ORDER-RESULT";
